...
| `GET /v1/graphql/schema` | No body. Generates SDL from currently applied `TableSchema` definitions. Rust SDK callers can use `TraceDbClient::graphql_schema`, `TraceDbClient::graphql_schema_typed`, or `TraceDbAsyncClient::graphql_schema_typed`; TypeScript SDK callers can use `TraceDB.graphqlSchema()`. | `GraphQlSchemaResponse` with `adapter`, generated `schema` SDL, `tables`, and `execution`, which states that execution still uses `POST /v1/graphql` and returns TraceDB's `QueryResponse`, not a GraphQL data envelope. This is schema export only. |
