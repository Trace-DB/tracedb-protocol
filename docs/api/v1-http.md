...
| `POST /v1/graphql` | `GraphQlQueryRequest`: `{ "query": string }`, where `query` is the bounded GraphQL adapter form with one root table field and arguments such as `tenant_id`, `where`, `match`, `near`, `limit`, `freshness`, and `explain`. | Same result shape as `POST /v1/query`: lean results by default, results plus `HybridExplain` when the GraphQL query sets `explain: true`. This is not GraphQL schema generation, mutation support, or resolver runtime. |
