...
BODYLESS_MANAGED_ROUTING_ROUTES = {
    ("get", "/v1/admin/jobs"),
}
...
ROUTES = [
    ...
    (
        ...
        "/v1/insert",
        "records",
        "Insert record compatibility route",
        "RecordInput",
        "EpochResponse",
        True,
    ),
    ...
    (
        ...
        "/v1/traceql",
        "query",
        "Run native TraceQL query or command",
        "TraceQlQueryRequest",
        "QueryResponse",
        False,
    ),
...
def components() -> dict[str, Any]:
    return {
        "schemas": {
            ...
            "TableSchema": object_schema(
                "TraceDB table schema. Names must use GraphQL-safe identifiers; schema apply rejects duplicate columns, overlapping scalar/text/vector columns, reserved TraceDB result metadata fields, and undeclared vector source columns before WAL append.",
                {
                    "name": {"type": "string"},
                    ...
                    "vector_columns": array_schema(
                        object_schema("Vector column schema.")
                    ),
                },
            ),
            "RecordInput": object_schema(
            ...
            "RecordPutRequest": object_schema(
            ...
            "RecordPatchRequest": object_schema(
            ...
            "RecordDeleteRequest": object_schema(
                "Delete/tombstone record request.",
                {
                    "table": {"type": "string"},
                    "tenant_id": {"type": "string"},
                    "id": {"type": "string"},
            ...
            "RecordGetRequest": object_schema(
                "Get record request.",
                {
                    "table": {"type": "string"},
                    "tenant_id": {"type": "string"},
            ...
            "RecordScanRequest": object_schema(
                "Scan records request.",
                {
                    "table": {"type": "string"},
                    "tenant_id": {"type": "string"},
            ...
            "TraceQlQueryRequest": object_schema(
                "Native TraceQL query request.",
                {
                    "query": {"type": "string"},
                },
            ),
            "GraphQlQueryRequest": object_schema(
            ...
            "GraphQlSchemaResponse": object_schema(
            ...
            "HybridScoreComponents": object_schema(
            ...
            "SnapshotRequest": object_schema(
                "Snapshot creation request.", {"target": {"type": "string"}}
            ),
            ...
            "DatabasesResponse": object_schema(
            ...
            "BranchesResponse": object_schema(
            ...
            "EpochResponse": object_schema(
                "Epoch allocation response.",
                {"epoch": {"type": "integer"}},
                additional_properties=False,
            ),
            "PutBatchResponse": object_schema(
                "Batch write response.",
                {
                    "epoch": {"type": "integer"},
                    "record_count": {"type": "integer"},
                    "write_timing": object_schema(
            ...
            "GetRecordResponse": object_schema(
                "Get record response.",
                {
            ...
            "RecordScanOutput": object_schema(
                "Scan output.",
                {
            ...
            "AccessPathExplain": object_schema(
            ...
            "QueryPhaseTiming": object_schema(
            ...
            "AccessPathTiming": object_schema(
            ...
            "CompactResponse": object_schema(
                "Compact response.",
                {"compacted": {"type": "boolean"}},
                additional_properties=False,
            ),
            "SnapshotResponse": object_schema(
                "Snapshot response.",
                {
            ...
            "RestoreVerification": object_schema(
...
def operation_id(method: str, path: str) -> str:
    ...
    return method + "".join(
        part.replace("-", "_").title().replace("_", "") for part in parts
    )


MANAGED_ROUTING_QUERY_PARAMS: list[dict[str, Any]] = [
    {
        "name": "database_id",
        "in": "query",
        "required": False,
        "schema": {"type": "string"},
        "description": (
            "Canonical managed-routing database identifier for bodyless routes. "
            "SDKs must use this parameter name (not db_id, databaseId, or similar variants) "
            "so all SDKs target the same gateway routing key."
        ),
    },
    {
        "name": "branch_id",
        ...
        "schema": {"type": "string"},
        "description": (
            "Canonical managed-routing branch identifier for bodyless routes. "
            "SDKs must use this parameter name (not br_id, branchId, or similar variants) "
            "so all SDKs target the same gateway routing key."
        ),
    },
]
...
def route_operation(
    ...
    is_deprecated = route_key in DEPRECATED_ROUTES
    has_bodyless_routing = route_key in BODYLESS_MANAGED_ROUTING_ROUTES
    ...
    operation: dict[str, Any] = {
        ...
        "responses": {
            "200": {
                ...
                "content": {
                    "application/json": {"schema": schema_ref(response_schema)}
                },
            },
            "400": {
    ...
    if is_deprecated:
        ...
        operation["description"] = (
            "Deprecated. Use POST /v1/records/put instead. "
            "This route remains for backwards compatibility and will be removed in a future release."
        )
    ...
    if has_bodyless_routing:
        operation.setdefault("parameters", [])
        operation["parameters"].extend(MANAGED_ROUTING_QUERY_PARAMS)
...
def validate_spec(spec: dict[str, Any]) -> None:
    ...
    dynamic_object_paths = [
        (
            ...
            schemas["GraphQlError"]["properties"]["extensions"],
        ),
        ("GraphQlResponse.data", schemas["GraphQlResponse"]["properties"]["data"]),
        ("RecordOutput.fields", schemas["RecordOutput"]["properties"]["fields"]),
        ("HybridQueryRow.fields", schemas["HybridQueryRow"]["properties"]["fields"]),
        (
            "PutBatchResponse.write_timing",
            schemas["PutBatchResponse"]["properties"]["write_timing"],
        ),
    ]
    for path, schema in dynamic_object_paths:
        if schema.get("additionalProperties") is False:
            errors.append(f"{path} must allow dynamic additional properties")
    ...
    if "POST /v1/records/:table" in insert_description:
        errors.append(
            "/v1/insert deprecation must not point to POST /v1/records/:table"
        )
...
def main() -> int:
    ...
    if args.check:
        ...
        if current != rendered:
            print(
                f"{ARTIFACT} is stale; run scripts/generate_openapi_v1.py", flush=True
            )
            return 1
        return 0
